...
You will now launch your very first experiment with the Tensorflow object detection API. Create a folder `training/reference`. Move the `pipeline_new.config` to this folder. 

Since the provided workspace has a storage limit of 3Gb, the checkpoints of the training will have to be stored somewhere else. For this purpose create a new folder in `/data/` for example `/data/training/reference/`

You will now have to launch two processes: 
* a training process:
```
python experiments/model_main_tf2.py --model_dir=/data/training/reference/ --pipeline_config_path=training/reference/pipeline_new.config
```
* an evaluation process:
```
python experiments/model_main_tf2.py --model_dir=/data/training/reference/ --pipeline_config_path=training/reference/pipeline_new.config --checkpoint_dir=/data/training/reference/
```
...
```
python3 -m tensorboard.main --logdir=/data/training/reference/
```
...
```
python .\exporter_main_v2.py --input_type image_tensor --pipeline_config_path training/experiment0/pipeline.config --trained_checkpoint_dir /data/training/reference/ckpt-50 --output_directory training/reference/exported_model/
```

Finally, you can create a video of your model's inferences for any tf record file. To do so, run the following command (modify it to your files):
```
python inference_video.py -labelmap_path label_map.pbtxt --model_path training/reference/exported_model/saved_model --tf_record_path /home/workspace/data/test/tf.record --config_path training/reference/pipeline_new.config --output_path animation.gif
```

## Submission Project Documentation
The project documentation can be found as a pdf with the name
`Project_Object_Detection_Murube.pdf`
 
Additionally, the generated animations can be found in the compressed zip file: `animation_day_night,zip`