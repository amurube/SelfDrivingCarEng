...
def get_paddings(array, pool_size, pool_stride):
    ...
    _, width, height, _ = array.shape
    wpad = (width // pool_stride) * pool_stride + pool_size - width
    hpad = (height // pool_stride) * pool_stride + pool_size - height
...
def get_output_size(shape, pool_size, pool_stride):
    ...
    """
    # IMPLEMENT THIS FUNCTION
    new_w = (shape[1] - pool_size)/pool_stride +1
    new_h = (shape[2] - pool_size)/pool_stride +1
...
if __name__ == '__main__':
    ...
    idx_i = 0
    for i in range(0, padded.shape[1], pool_stride):
        idx_j = 0
        for j in range(0, padded.shape[2], pool_stride):
            output[:,idx_i,idx_j,:] = np.max(padded[:,i:i+pool_stride, j:j+pool_stride, :], axis = (1,2))
            idx_j += 1
        idx_i += 1
    check_output(output)
